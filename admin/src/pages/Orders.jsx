      ...
      <div>
        {
          orders.map((order, index) => (
            <div className='grid grid-cols-1 sm:grid-cols-[0.5fr_2fr_1fr] lg:grid-cols-[0.5fr_2fr_1fr_1fr_1fr] gap-3 items-start border-2 border-gray-200 p-5 md:p-8 my-3 md:my-4 text-xs sm:text-sm text-gray-700' key={index}>
              ...
              <div>
                <p className='text-sm sm:text-[15px]'>จำนวนสินค้า : {order.items.length}</p>
                <p className='mt-3'>วิธีชำระเงิน : {order.paymentMethod}</p>
                <p>สถานะการชำระเงิน : { order.payment ? 'สำเร็จ' : 'รอดำเนินการ' }</p>
                <p>วันที่ : {new Date(order.date).toLocaleDateString()}</p>
              </div>
              <p className='text-sm sm:text-[15px]'>{currency}{order.amount}</p>
              <select onChange={(event)=>statusHandler(event,order._id)} value={order.status} className='p-2 font-semibold'>
                <option value="Order Placed">คำสั่งซื้อสำเร็จ</option>
